...
class Calib(object):
    ...
    def get_velo2cam(self, num):
        return np.dot(self.get_proj(num), self.get_velo2rect() )

    def get_cam2velo(self, num):
        k = self.get_velo2cam(0)
        k = np.vstack((k, np.array([0,0,0,1])))
        r = np.linalg.inv(k)
        return r[:,:3]

    def proj_to_img(self, points, proj_matrix):
        """
        ...
        points = proj_matrix @ points
        points[:2, :] /= points[2, :]
        return points[:2, :]

    def proj_to_pointcloud(self, points, proj_matrix):
        """
        Apply the perspective projection
        Args:
            points:     2D points in camera coordinate [2, npoints]
            proj_mat:   Projection matrix [4, 3]
        """
        print(proj_matrix.shape)
